...
def sim_run(options, RUN):
    ...
    for i in range(1,sim_total+1):
        ...
        arr = np.array(time_a)
    time_aa = arr.reshape(sim_total+1,1)
    print("time is " + str(time_aa))
    print('The state data is stored as [x_t, y_t, psi_t, v_t] in state_i \n And the appended array is' + str(state_i))
    print('The pedal and steering data is stored as [pedal, steering] in u_i \n And the appended array is' + str(u_i))
    
    print('size of \n time scale is {t} and \n size of input u_i is {u} and \n state {s}'.format(t= len(time_aa), u=len(u_i), s= len(state_i)))
    plt.plot(time_aa, u_i)
    plt.plot(time_aa, state_i)
    plt.grid(True)
    # time_a = numpy.transpose(time_a)
    
    # plt.set_title('All in one')
    ...
    def update_plot(num):
        # Car.
        [xx,yy] = car_patch_pos(state_i[num,0], state_i[num,1], state_i[num,2])
        patch_car.set_xy(car_patch_pos(state_i[num,0], state_i[num,1], state_i[num,2]))
        # print("The position of the car is xx = " xx) + "and yy is" + float(yy)
        # print("The position of the car is x = {x:1.3f} and y = {y:1.3f}".format(x=xx, y = yy))
        patch_car.angle = np.rad2deg(state_i[num,2])-90
        # Car wheels  
        strng_angle = u_i[num,1]*2
        steering_wheel(u_i[num,1]*2)
        # print("The steering angle is = {x:1.3f} ".format(x=strng_angle))
